...
function LibraryBookDetail() {
  ...
  const handleConfirmReservation = async (borrowDate) => {
    try {
      ...
      // Kiểm tra nếu borrowDate nhỏ hơn thời gian hiện tại
      if (borrowDate < now) {
        throw new Error("thời gian đặt phải lớn hơn hoặc bằng thời gian hiện tại.");
      }
      const user = JSON.parse(localStorage.getItem("user"));
      if (!user) {
      ...
      console.log(error);
      console.error("Đặt sách thất bại," + error.message);
      alert("Đặt sách thất bại, " + error.message);
    }
  };
